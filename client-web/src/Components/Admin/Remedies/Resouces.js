...
const Resources = () => {
  ...
  useEffect(() => {
    const fetchReasons = async () => {
      try {
        const response = await fetch(`${ip}/getReason`);
  ...
  const onSubmit = async (values, action) => {
    ...
    fetch(`${ip}/addResource?id=${filesData.reason}`, requestOptions)
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="mx-5 mt-3 space-y-2">
              <div className="flex">
                ...
                <label
                  htmlFor="pptFiles"
                  className="font-semibold text-gray-500 mx-3"
                >
                  Choose PPT Files:
                </label>
                <input
